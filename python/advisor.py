...
def map_glacier_tx(tx: dict) -> dict:
    # Glacier's raw fields (txHash, blockTimestamp) don't match
    # normalize.py's expected shape (hash, timestamp). Map into the
    # canonical shape here, at the fetch layer, rather than teaching
    # normalize.py multiple raw shapes.
    return {
        "hash": tx.get("txHash"),
        "value": tx.get("value"),
...
async def get_wallet_history(wallet_address: str) -> list:
    ...
    return normalize_many([map_glacier_tx(t) for t in data.get("transactions", [])])
